...
bool ondrag;							// 是否开始了拖动图片状态
bool dragging;							// 是否在拖动图片状态（确定帧率）
bool ondragzoom;						// 是否开始了拖动放大图片状态
bool draggingzoom;						// 是否在拖动放大图片状态（确定帧率）
...
//set flag:
#define SF_SR(RETVAL)	{ surfrenew = RETVAL;if(surfrenew) surfrenewtick = GetTickCount(); }//surfrenew
#define SF_SRFR(RETVAL) { surfrenew = RETVAL;if(surfrenew) surfrenewtick = GetTickCount(); needforcerenew |= surfrenew;}//surfrenew&needforcerenew
#define SF_DR()			{ dragging = true;dragtick = GetTickCount(); }//dragging
#define SF_OZ()			{ onzoom = true;zoomtick = GetTickCount(); }//onzoom
...
RECT textrect_pic;						// 图片信息区域
RECT textrect_surface;					// surface信息区域
RECT textrect_flag;						// 状态信息区域
RECT textrect_picnum;					// 图片状态显示区域
...
bool FullScreen_Windowed(bool tofull, bool winchange = true);
...
void SetSurface(PicPack *newpicpack, PicPack *oldpicpack);	// 设置图片之后设置surface
